...
async def handler(ws):
    # Optional token auth: if ZS_BRIDGE_TOKEN is set, the FIRST message must carry {"token":"..."}
    # or the WS URL query ?token=xxx (for tunnels). Reject silently with error JSON then close.
    if BRIDGE_TOKEN:
        # check query string token (works for wss://domain/?token=xxx via tunnel)
        try:
            path = getattr(ws, "path", "") or getattr(getattr(ws, "request", None), "target", "") or ""
            if f"token={BRIDGE_TOKEN}" not in path:
                # also allow token via first message - wait 3s
                try:
                    ...
                    if m.get("type") not in ("ping","call_tool","list_tools","studio_status"):
                        # auth ok, proceed to normal loop (this message was just auth)
                        pass
                    else:
                        # it was a real message with token field, process it below - re-inject
                        # simplest: handle it inline then continue
                        msg = m
                        # fall through to normal processing for this first msg - we simulate by handling then continuing loop
                        # To keep code simple, we just log and continue; client will retry the call
                        log(f"token auth ok from {getattr(ws,'remote_address',('?',))[0]}", "gr")
                except asyncio.TimeoutError:
                    await ws.send(json.dumps({"type":"error","error":"auth timeout - send token in first message or ?token=xxx"}))
                    await ws.close(code=1008)
                    return
            else:
                log(f"token auth via URL ok from {getattr(ws,'remote_address',('?',))[0]}", "gr")
        except Exception as e:
            log(f"auth check error: {e}", "rd")
